    ...
    _connect() {
        ...
        this._socket.on('ready', () => {
            ...
            if (this.context.version === '3.4') {
                ...
                this._socket._sessionKeyTimeout = setTimeout(() => {
                    ...
                    this.log.debug('Attempting fallback to 3.4-style session key negotiation');
                    
                    // Clear the timeout to avoid conflicts
                    if (this._socket._sessionKeyTimeout) {
                        clearTimeout(this._socket._sessionKeyTimeout);
                        this._socket._sessionKeyTimeout = null;
                    }
                    
                    // Switch to 3.4 message handler for the fallback
                    this._msgQueue = async.queue(this._msgHandler_3_4.bind(this), 1);
                    this.log.debug('Switched to 3.4 message handler for fallback');
                    
                    // Try sending session key negotiation using 3.4 format as fallback
                    try {
                        // Use the same payload format as regular 3.4 initialization
                        const payload = {
                            data: this._tmpLocalKey,
                            encrypted: true,
                            cmd: 3 //CommandType.BIND
                        }
                        this._send_3_4(payload);
                        this.log.debug('Sent fallback 3.4-style session key negotiation');
                        
                        // Set another timeout for the fallback
                        this._socket._sessionKeyTimeout = setTimeout(() => {
                            this.log.error('Both 3.5 and 3.4-style session key negotiation failed');
                            this._socket.emit('error', new Error('ERR_SESSION_KEY_TIMEOUT'));
                        }, 5000);
                    } catch (error) {
                        this.log.error(`Fallback session key negotiation also failed: ${error.message}`);
                        this._socket.emit('error', new Error('ERR_SESSION_KEY_TIMEOUT'));
                    }
        ...
        this._socket.on('data', msg => {
            ...
            do {
                ...
                // Check for Tuya 3.5 packet format (00006699...00009966)
                ...
                // Check for classic Tuya packet format (000055aa...0000aa55)
                if (startingIndex === -1) {
